...
void TakeGrantPanel::setup(QStringList users, QStringList permissions)
{
    ...
    ui->listWidget_create_permissions->clear();
    ...
    ui->listWidget_create_permissions->addItems(permissions);
}
...
{
    if (ui->listWidget_create_permissions->currentRow() == -1 || ui->lineEdit_create_username->text() == "")
    {
        return;
    }
    QString username = ui->lineEdit_create_username->text();
    QStringList permissions;
    for (qsizetype i = 0; i < ui->listWidget_create_permissions->selectedItems().size(); i++)
    {
        permissions.push_back(ui->listWidget_create_permissions->selectedItems()[i]->text());
    }
